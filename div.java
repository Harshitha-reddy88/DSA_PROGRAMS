...
public class Temp2 {
    ...
    {
            while(startpoint<str.length()-5)
              {
                 if(str.charAt(startpoint)== '<' && str.charAt(startpoint+1)== 'd' &&  str.charAt(startpoint+2)== 'i'  && str.charAt(startpoint+3)== 'v' && str.charAt(startpoint+4)== '>')
                 {
                    return startpoint+5;
                 } 
                    startpoint++;
     ...
     public static int findParagraphend(int end,String str)
     {
          while(end<str.length()-6)
              {
                 if(str.charAt(end)== '<' && str.charAt(end+1)== '/'  && str.charAt(end+2)== 'd' && str.charAt(end+3)=='i' && str.charAt(end+3)=='v' && str.charAt(end+3)=='>')
                 {
                    return end+6;
                 } 
                    end++;
